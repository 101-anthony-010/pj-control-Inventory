...
const AddInfoUser = ({ handleChangeIsShowInfoUser, user }) => {
  ...
  return (
    <section className='max-w-5xl relative bg-white rounded-md p-8'>
      <button onClick={() => handleChangeIsShowInfoUser()} className='absolute top-0 right-0 p-2'>
      ...
      <form onSubmit={handleSubmit(submit)} className='grid grid-cols-2 w-full gap-4' action="">
        ...
        <button className='col-span-2 bg-green-400 rounded-md font-semibold p-2 w-[250px] m-auto shadow-md'>Agregar</button>
      </form>
    </section>
  )
}
